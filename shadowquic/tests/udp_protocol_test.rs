...
fn test_socks5_udp_dns_query_to_1_1_1_1() {
    ...
    println!("1. DNS query for www.youtube.com: {} bytes", dns_query.len());
...
async fn test_end_to_end_udp_dns_flow() {
    ...
    match decoded {
        Frame::UdpData(data) => {
            ...
            println!("[Server] Sending {} bytes back to client\n", response_buf.len());
            ...
            match response_decoded {
                Frame::UdpData(resp_data) => {
                    println!("[Client] Received DNS response: {} bytes", resp_data.payload.len());
