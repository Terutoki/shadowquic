...
impl SocksServer {
    ...
    /// Keep TCP control connection alive during UDP sessions
    /// This prevents the connection from being closed unexpectedly
    async fn keep_tcp_alive(mut stream: TcpStream) {
        let mut buf = vec![0u8; 1];
        loop {
            match stream.read(&mut buf).await {
                Ok(0) => {
                    trace!("TCP control connection closed by client");
                    break;
                }
                Ok(_) => {
                    // Data received, but we ignore it - just keep the connection alive
                    trace!("Received control data on UDP Associate TCP connection");
                }
                Err(e) => {
                    trace!("Error reading from TCP control connection: {{}}", e);
                    break;
                }
            }
        }
    ...
    async fn accept(&mut self) -> Result<ProxyRequest, SError> {
        ...
        match req.cmd {
            ...
            SOCKS5_CMD_UDP_ASSOCIATE => {
                ...
                // Spawn background task to keep TCP control connection alive
                tokio::spawn(Self::keep_tcp_alive(s));
